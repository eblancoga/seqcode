...
int main (int argc, char *argv[])
{
  ...
  if (ValidationUserName(UserName) == TRUE)
	...
	{
	  sprintf(OutputDirName,"%s/%s_GENEplot_%d",Prefix,UserName,L);
	  ...
	  /* Output files */
	  sprintf(OutputFileName,"%s/GENEprofile_%s_%d.txt",OutputDirName,UserName,L);
	  sprintf(mess,"Filename for the GENE average profile %s",OutputFileName);
	  printRes(mess);

	  if (FULL)
	    {
	      sprintf(OutputFileName2,"%s/FULLprofile_%s_%d.txt",OutputDirName,UserName,L);
	      sprintf(mess,"Filename for the TSS full profile %s",OutputFileName2);
	      printRes(mess);
	    }
	  
	  sprintf(PlotFileName,"%s/PlotGENEprofile_%s_%d.pdf",OutputDirName,UserName,L);
  ...
  if (m->nGenes>0)
    {
      ...
      sprintf(RscriptFileName,"%s/RscriptGENEprofile_%s_%d.txt",OutputDirName,UserName,L);
      sprintf(mess,"Preparing the Rscript file %s",RscriptFileName);
      printRes(mess);
      
      CreateRscriptGENE(OutputFileName,RscriptFileName,PlotFileName,UserName,L,m->nReads);
