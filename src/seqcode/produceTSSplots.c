...
int main (int argc, char *argv[])
{
  ...
  if (ValidationUserName(UserName) == TRUE)
	...
	{
	  sprintf(OutputDirName,"%s/%s_TSSplot_%d",Prefix,UserName,L);
	  ...
	  /* Output files */
	  sprintf(OutputFileName,"%s/TSSprofile_%s_%d.txt",OutputDirName,UserName,L);
	  ...
	  sprintf(PlotFileName,"%s/PlotTSSprofile_%s_%d.pdf",OutputDirName,UserName,L);
  ...
  if (m->nGenes>0)
    {
      ...
      sprintf(RscriptFileName,"%s/RscriptTSSprofile_%s_%d.txt",OutputDirName,UserName,L);
