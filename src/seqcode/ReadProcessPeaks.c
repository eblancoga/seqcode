/*************************************************************************
*                                                                        *
*   Module: ReadProcessPeaks                                             *
*                                                                        *
*   Reading BED file, producing the REGION plot                          *
*                                                                        *
		      ...
		      long nreads)
{
  ...
  while(fgets(line,MAXLINE,file)!=NULL)
    {
	  ...
	  if (line1 == NULL || line2 == NULL || line3 == NULL)
	    {
	      sprintf(mess,
		      "Unexpected format for <chr,pos1,pos2,score> in the line :\n-->%s\n",
		      lineCopy);
	      printError(mess);
	    }
	  ...
	  if ((pos1-L)/WINDOWRES<0 || (pos2+L)/WINDOWRES>=ChrSizes[key]/WINDOWRES)
	      ...
	      for(i=(maxCenter-L)/WINDOWRES,j=0; i<(maxCenter+L)/WINDOWRES; i++, j=j+WINDOWRES)
		{
	      ...
	      fprintf(outfile,"%s\t%ld\t%ld\t%s\t%ld\t%d\n",
  ...
  OutputAvgReadsFile(REGION,L,OutputFileName,nPeaks,nreads);
