/*************************************************************************
*                                                                        *
*   Module: ProcessTargetGenesTESMaps                                    *
*                                                                        *
*   Process target genes to generate the TES maps                        *
*                                                                        *
...
/* Process target genes to produce the TES heat maps */
	  ...
	  if (found)
	      ...
	      if (Transcripts[key][i].strand=='+')
		{
		  ...
		  if ((tes-L)/WINDOWRES<0 || (tes+L)/WINDOWRES>=ChrSizes[key]/WINDOWRES)
			...
			{
			  /* TES heatmap plot*/
			  AUXREGION[i-((tes-L)/WINDOWRES)] = READS[key][i];
			}

		      /* Output this gene into the HEAT matrix output file */
		      OutputOneGeneHeatFile(gene,
					    AUXREGION,
					    L,nreads,
					    outfile); 
	      ...
	      else
		{
		  ...
		  if ((tes-L)/WINDOWRES<0 || (tes+L)/WINDOWRES>=ChrSizes[key]/WINDOWRES)
			...
			{
			  /* TES heatmap plot*/
			  AUXREGION[((tes+L)/WINDOWRES)-i] = READS[key][i];
			}		  
